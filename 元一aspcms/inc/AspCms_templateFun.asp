...
Function makeAbout(Byval sortID, Byval page, Byval isMakeHtml)
	...
	if isnul(rsObj("ImagePath")) then
		templateObj.content=replace(templateObj.content,"[about:pics]",sitePath&"/images/nopic.gif" )
	else 
		if instr(rsObj("IndexImage"),"http://")>0 then 
			templateObj.content=replace(templateObj.content,"[about:pics]",rsObj("ImagePath"))
		else
			templateObj.content=replace(templateObj.content,"[about:pics]",rsObj("ImagePath"))
		end if
	end if
